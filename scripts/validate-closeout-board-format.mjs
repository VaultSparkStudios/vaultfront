...
const targetPath =
  positional[0] || path.join(ROOT, "docs", "CLOSEOUT_STATUS_BOARD.md");
...
export function validateCloseoutBoard(body) {
  const missingRequired = REQUIRED_BLOCKS.filter(
    (block) => !block.pattern.test(body),
...
async function main() {
  ...
  if (JSON_MODE) {
    ...
    if (!fail) {
      console.log(
        "  ✓   conformant — all required canonical closeout blocks present",
      );
    } else {
      console.log("");
      console.log(
        "  Repair path: regenerate the closeout status board in canonical box-drawing format.",
      );
    }
  }

...
if (!planModeRequired) {
  emit({
    status: "not_required",
    tier,
    reason: "tier does not require plan-mode",
  });
  process.exit(0);
}
...
try {
  if (fs.existsSync(projectsDir)) {
    ...
    const dirs = fs
      ...
      .filter(
        (d) =>
          d.includes("vaultspark-studio-ops") ||
          d.includes(path.basename(ROOT)),
      );
    for (const d of dirs) {
...
const sig3 = !!(
  ...
  new Date(status.planModeLastActivatedAt).getTime() >= sessionStart
);
...
const result = {
  ...
  reminder: active
    ? null
    : `Run /model opusplan to activate Opus-plans-Sonnet-executes. Then: node scripts/mark-plan-mode.mjs`,
};
...
function emit(r) {
  ...
  const tag =
    { active: "✓ ACTIVE", missing: "⚠ MISSING", not_required: "= N/A" }[
      r.status
    ] || r.status;

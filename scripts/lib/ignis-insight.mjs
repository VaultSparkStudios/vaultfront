...
export function loadIgnisInsight(options = {}) {
  ...
  const generated =
    core.match(/Last synthesised:\s*(\d{4}-\d{2}-\d{2})/)?.[1] ?? null;

  const phase =
    statusBlock.match(/IGNIS Phase:\s*([^\n]+)/)?.[1]?.trim() ?? null;
  const avgIq =
    statusBlock.match(/Average tracked IQ:\s*([^(\n]+)/)?.[1]?.trim() ?? null;
  const topProject =
    statusBlock.match(/Top tracked project:\s*([^\n]+)/)?.[1]?.trim() ?? null;
  const topRisk =
    statusBlock
      .match(/Highest current founder-facing risk:\s*([^\n]+)/)?.[1]
      ?.trim() ?? null;
  const truthMix =
    statusBlock.match(/Truth status mix:\s*([^\n]+)/)?.[1]?.trim() ?? null;
  const coverage =
    statusBlock.match(/Tracked IGNIS coverage:\s*([^\n]+)/)?.[1]?.trim() ??
    null;

  // Top recommendation: first numbered line in Recommended Actions.
  const firstAction =
    actionsBlock.match(/^\s*1\.\s+(.+)$/m)?.[1]?.trim() ?? null;
  ...
  const daysSinceSynth = generated
    ? Math.max(
        0,
        Math.floor((Date.now() - new Date(generated).getTime()) / 86400000),
      )
    : null;
...
if (process.argv[1]?.endsWith("ignis-insight.mjs")) {
  ...
  if (process.argv.includes("--json")) {
    ...
    console.log(
      `IGNIS Insight · synth ${out.generated} (${out.daysSinceSynth}d)`,
    );

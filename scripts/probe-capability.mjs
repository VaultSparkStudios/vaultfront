...
import { getSecret, redact, resolveCapability } from "./lib/secrets.mjs";
...
const PROBES = {
  ...
  "stripe.checkout": async () => {
    ...
    const r = await httpFetch("https://api.stripe.com/v1/balance", {
  ...
  "cloudflare.r2": async () => {
    ...
    if (!token)
      return {
        ok: false,
        status: "skipped",
        detail: "no CF token to verify R2 account",
      };
  ...
  "resend.email": async () => {
    ...
    const r = await httpFetch("https://api.resend.com/domains", {
      headers: { Authorization: `Bearer ${key}` },
    });
    return interpret(r);
  },
  "github.api": async () => {
    const key =
      getSecret("GITHUB_TOKEN", "github.api") ||
      getSecret("GH_TOKEN", "github.api");
    const r = await httpFetch("https://api.github.com/user", {
      headers: {
  ...
  "supabase.admin": async () => {
    ...
    const key =
      getSecret("SUPABASE_SERVICE_KEY", "supabase.admin") ||
    ...
    const r = await httpFetch(`${url.replace(/\/$/, "")}/rest/v1/`, {
  ...
  "posthog.api": async () => {
    const key =
      getSecret("POSTHOG_PERSONAL_API_KEY", "posthog.api") ||
      getSecret("POSTHOG_API_KEY", "posthog.api");
    const r = await httpFetch("https://app.posthog.com/api/users/@me/", {
      headers: { Authorization: `Bearer ${key}` },
    });
    return interpret(r);
  },
  "hetzner.cloud": async () => {
    ...
    const r = await httpFetch("https://api.hetzner.cloud/v1/locations", {
      headers: { Authorization: `Bearer ${key}` },
    });
    return interpret(r);
  },
  "brevo.email": async () => {
    ...
    const r = await httpFetch("https://api.brevo.com/v3/account", {
      headers: { "api-key": key },
    });
    return interpret(r);
  },
  "namecheap.api": async () => {
    return {
      ok: false,
      status: "skipped",
      detail: "Namecheap API requires per-call IP allowlist — probe not safe",
    };
  },
  "google.gmail": async () => {
    ...
    return {
      ok: false,
      status: "skipped",
      detail: "OAuth bearer; probed via scripts/test-gmail-send.mjs",
    };
  },
  "google.calendar": async () => {
    return {
      ok: false,
      status: "skipped",
      detail: "OAuth bearer; probed via scripts/test-calendar-read.mjs",
    };
  },
  "google.drive": async () => {
    return {
      ok: false,
      status: "skipped",
      detail: "OAuth bearer; probed via scripts/test-drive-read.mjs",
    };
  },
};
...
function interpret(r) {
  ...
  if (r.status >= 500)
    return { ok: false, status: "unreachable", detail: `HTTP ${r.status}` };
  if (r.status >= 200 && r.status < 300)
    return { ok: true, status: "ok", detail: `HTTP ${r.status}` };
  return { ok: false, status: "auth-error", detail: `HTTP ${r.status}` };
}
...
try {
  ...
  if (e.code === "ENOENT") {
    ...
    console.error(
      "⚠ secrets/CAPABILITY_MAP.json missing — probe skipped (SKIPPED=no-capability-map)",
    );
    process.exit(0);
  }
  throw e;
...
for (const cap of caps) {
  ...
  if (!resolved.ok) {
    results.push({
      cap,
      status: "skipped",
      detail: `missing env: ${resolved.missing.join(", ")}`,
...
try {
  fs.writeFileSync(CAP_MAP_PATH, JSON.stringify(capMap, null, 2) + "\n");
} catch {
  /* non-fatal */
}

...
function exitWith(obj, code = 0) {
  if (JSON_MODE) console.log(JSON.stringify(obj, null, 2));
  else console.log(obj.message || obj);
  process.exit(code);
}
...
if (LIST) {
  ...
  const rows = Object.entries(capMap.capabilities)
    .map(([cap, def]) => {
  ...
  for (const r of rows) {
    ...
    console.log(
      `  · ${r.cap.padEnd(28)} missing: ${r.missing.join(", ")}${tag}`,
    );
  }
  console.log(
...
if (!def)
  exitWith(
    {
      ok: false,
      message: `unknown capability "${CAP}" — add it to secrets/CAPABILITY_MAP.json first`,
    },
    1,
  );
const required = def.env || [];
if (!required.length)
  exitWith(
    {
      ok: false,
      message: `capability "${CAP}" has no required env vars declared`,
    },
    1,
  );

// Source file: secrets/<cap>-paste.txt by default; accept secrets/<cap>.txt as fallback.
const candidateSources = SOURCE_OVERRIDE
  ? [SOURCE_OVERRIDE]
  : [
...
const existing = fs.existsSync(envTarget)
  ? parseEnv(fs.readFileSync(envTarget, "utf8"))
  : {};
...
function mergeEnvFiles() {
  ...
  for (const f of fs.readdirSync(SECRETS_DIR)) {
    ...
    Object.assign(
      out,
      parseEnv(fs.readFileSync(path.join(SECRETS_DIR, f), "utf8")),
    );
  }
  return out;
}
...
function extractKeys(raw, required, cap) {
  ...
  try {
    ...
    for (const k of required) {
      ...
      const found = Object.entries(j).find(
  ...
  for (const rawLine of raw.split(/\r?\n/)) {
    ...
    for (const [labelOrPrefix, targetKey] of Object.entries(aliases)) {
      ...
      if (
        line.toLowerCase().startsWith(labelOrPrefix.toLowerCase()) &&
        /[:=]/.test(line)
      ) {

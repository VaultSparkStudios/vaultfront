...
if (isDirect) {
  ...
  console.log(
    `classify-session-intent · ${result.intent.toUpperCase()} · prefers ${result.intentModel}`,
  );
  console.log(
    `  scores: planning=${result.scores.planning} execution=${result.scores.execution} exploration=${result.scores.exploration} ops=${result.scores.ops}`,
  );
}

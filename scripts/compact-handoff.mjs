...
if (trim) {
  ...
  const header = sections[0].trimStart().startsWith("## Where We Left Off")
    ? ""
    : sections.shift();
  ...
  if (sessions.length <= KEEP) {
    console.log(
      `= LATEST_HANDOFF.md already has ${sessions.length} session(s) — no trim needed.`,
    );
    process.exit(0);
  }
  ...
  console.log(
    `✂ Trimming LATEST_HANDOFF.md: keeping ${KEEP} sessions, archiving ${toArchive.length}`,
  );
  toArchive.forEach((s) => {
    const firstLine = s
      .split("\n")[0]
      .replace("## Where We Left Off", "")
      .trim();
  ...
  if (!dryRun) {
    ...
    console.log(
      `✓ ${toArchive.length} session(s) appended to context/HANDOFF_ARCHIVE.md`,
    );
  } else {
    console.log(
      `(dry-run) would rewrite LATEST_HANDOFF.md + append ${toArchive.length} to HANDOFF_ARCHIVE.md`,
    );
  }
  process.exit(0);
}
...
const stillFresh =
  cached && cached.hash === hash && Date.now() - (cached.ts || 0) < 3_600_000;
if (stillFresh && !force) {
  fs.writeFileSync(OUT, cached.digest);
  console.log(
    `✓ Compact handoff (cached) → context/LATEST_HANDOFF.compact.md  (${cached.digest.length} chars)`,
  );
  process.exit(0);
}
...
fs.writeFileSync(
  CACHE,
  JSON.stringify({ hash, digest: out, ts: Date.now() }, null, 2),
);

console.log(
  `✓ Compact handoff → context/LATEST_HANDOFF.compact.md  (${out.length} chars, cached 1h)`,
);
console.log(
  `  Tokens: input ${resp.usage?.input_tokens || 0}  output ${resp.usage?.output_tokens || 0}  cache_read ${resp.usage?.cache_read_input_tokens || 0}`,
);

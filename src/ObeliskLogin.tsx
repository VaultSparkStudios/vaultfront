...
export function ObeliskLogin({
  ...
  useEffect(() => {
    ...
    s.dataset.obeliskIdp = IDP;
    s.dataset.obeliskProject = project;
    s.dataset.obeliskTier = tier;
    s.dataset.obeliskReturn = ret;
  ...
  return (
    ...
    >
      ...
      <p style={{ opacity: 0.7, fontSize: 14, margin: "0 0 22px" }}>
        One account. Every VaultSpark game. Your progress, carried.
      </p>
      <button
        data-obelisk-signin
        style={{
      ...
      >
        Can't sign in? Recover access
      </button>
      <div style={{ marginTop: 14, fontSize: 11, opacity: 0.45 }}>
        Secured by Obelisk · passwordless · lost your device? recover with a
        backup code
      </div>
    </div>
  );

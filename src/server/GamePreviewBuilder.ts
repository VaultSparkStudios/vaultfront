...
export function buildPreview(
  ...
  if (isFinished) {
    ...
    if (isPrivate) {
      ...
      if (gc?.vaultSitesEnabled) gameOptions.push("Vault Sites");
      if (gc?.intelOperationsEnabled) gameOptions.push("Defense Factories");
      if (gc?.nations === "disabled") gameOptions.push("Nations Disabled");
      if (gc?.donateTroops) gameOptions.push("Troop Donations Enabled");
      if (gc?.nationGoldTroopEmphasis !== undefined) {
        gameOptions.push(`Nation Focus ${gc.nationGoldTroopEmphasis}/100`);
      }

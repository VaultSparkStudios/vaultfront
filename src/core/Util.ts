...
export function createPartialGameRecord(
  ...
  const record: PartialGameRecord = {
    ...
    version: "v0.0.2",
    turns,
    replay: buildReplayMetadata(turns, 100),
    telemetry,
  };
  return record;
}
...
function buildReplayMetadata(
  ...
  for (const turn of turns) {
    const intentsLength = turn.intents.length;
    for (const intent of turn.intents) {
      intentCounts[intent.type] = (intentCounts[intent.type] ?? 0) + 1;
    }
    if (
      turn.turnNumber % snapshotIntervalTurns === 0 ||
      turn.hash !== undefined
    ) {
      snapshots.push({
        turnNumber: turn.turnNumber,
        hash: turn.hash ?? null,
        intentsOffset: offset,
        intentsLength,
      });
    }
    offset += intentsLength;
  }
  return {
    snapshotIntervalTurns,
    snapshots,
    intentCounts,
  };
}

...
export class UnitView {
  ...
  troops(): number {
    if (this.gameView.isPlayerMaskedFromViewer(this.owner())) {
      return 0;
    }
...
export class PlayerView {
  ...
  outgoingAttacks(): AttackUpdate[] {
    if (this.game.isPlayerMaskedFromViewer(this)) {
      return this.data.outgoingAttacks.map((attack) => ({
        ...attack,
        troops: 0,
      }));
    }
  ...
  goldTroopFocus(): number {
    return this.data.goldTroopFocus ?? 50;
  }
  ...
  isTroopIntelHiddenToViewer(): boolean {
    return this.game.isPlayerMaskedFromViewer(this);
  }
...
export class GameView implements GameMap {
  ...
  private maskedTroopUntilByPlayer = new Map<number, Tick>();
  ...
  public update(gu: GameUpdateViewData) {
    ...
    const vaultStatusUpdates = gu.updates[
      GameUpdateType.VaultFrontStatus
    ] as VaultFrontStatusUpdate[];
    if (vaultStatusUpdates.length > 0) {
      const latestStatus = vaultStatusUpdates[vaultStatusUpdates.length - 1];
      this.maskedTroopUntilByPlayer.clear();
      for (const beaconState of latestStatus.beacons) {
        this.maskedTroopUntilByPlayer.set(
          beaconState.playerID,
          beaconState.maskedUntilTick,
        );
      }
    }
  ...
  troopMaskUntilTick(playerSmallID: number): Tick {
    return this.maskedTroopUntilByPlayer.get(playerSmallID) ?? 0;
  }

  isPlayerMaskedFromViewer(player: PlayerView): boolean {
    const myPlayer = this.myPlayer();
    if (!myPlayer) {
      return false;
    }
    if (player.smallID() === myPlayer.smallID()) {
      return false;
    }
    const maskedUntil = this.troopMaskUntilTick(player.smallID());
    return this.ticks() < maskedUntil;
  }

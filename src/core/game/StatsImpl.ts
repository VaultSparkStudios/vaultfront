...
export class StatsImpl implements Stats {
  ...
  private _addVaultFront(
    player: Player,
    key:
      | "vaultCaptures"
      | "vaultPassiveGold"
      | "vaultPassivePayouts"
      | "vaultConvoysLaunched"
      | "vaultConvoysDelivered"
      | "vaultConvoysIntercepted"
      | "vaultConvoysLost"
      | "defenseFactoryPulses"
      | "defenseFactoryPulseUptimeTicks"
      | "focusChanges"
      | "focusSamples"
      | "focusTotal"
      | "convoyReroutes"
      | "convoyEscortCommands"
      | "jamBreakerUses"
      | "surgeActivations"
      | "minute8Behind"
      | "vaultInteractions"
  ...
  vaultPassiveGold(player: Player, gold: BigIntLike): void {
    this._addVaultFront(player, "vaultPassiveGold", gold);
    this._addVaultFront(player, "vaultPassivePayouts", 1);
  }
  ...
  resourceFocusSet(player: Player, focus: number): void {
    this._addVaultFront(player, "focusChanges", 1);
    this._addVaultFront(player, "focusTotal", focus);
    this._addVaultFront(player, "focusSamples", 1);
  }
  ...
  vaultConvoyRerouted(player: Player): void {
    this._addVaultFront(player, "convoyReroutes", 1);
    this._addVaultFront(player, "vaultInteractions", 1);
  }

  vaultConvoyEscortCommand(player: Player): void {
    this._addVaultFront(player, "convoyEscortCommands", 1);
  }

  defenseFactoryJamBreaker(player: Player): void {
    this._addVaultFront(player, "jamBreakerUses", 1);
  }

  comebackSurgeActivated(player: Player): void {
    this._addVaultFront(player, "surgeActivations", 1);
  }

  minute8Behind(player: Player): void {
    this._addVaultFront(player, "minute8Behind", 1);
  }

  vaultInteraction(player: Player): void {
    this._addVaultFront(player, "vaultInteractions", 1);
  }

  cleanExecutionStreak(player: Player): void {
    this._addVaultFront(player, "cleanExecutionStreaks", 1);
  }

  squadObjectiveCompleted(player: Player): void {
    this._addVaultFront(player, "squadObjectiveCompletions", 1);
  }

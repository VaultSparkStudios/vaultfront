...
export class DefaultConfig implements Config {
  ...
  private lastValidVaultTuning: VaultConvoyRewardTuning | null = null;
  constructor(
  ...
  vaultSitesEnabled(): boolean {
    return this._gameConfig.vaultSitesEnabled ?? false;
  }
  intelOperationsEnabled(): boolean {
    return this._gameConfig.intelOperationsEnabled ?? false;
  }
  vaultConvoyRewardTuning(): VaultConvoyRewardTuning {
    ...
    const raw = this._gameConfig.vaultConvoyRewardTuning as
      | Partial<VaultConvoyRewardTuning>
      | { version?: string; values?: Partial<VaultConvoyRewardTuning> }
      | undefined;
    ...
    if (
      ...
      console.warn(
        "[vaultfront] unsupported tuning payload version, rolling back to last valid tuning",
      );
    }
    const resolved = {
      ...base,
    ...
    const finite = Object.values(resolved).every(
      (value) => typeof value === "number" && Number.isFinite(value),
    );
    const coherent =
      resolved.rewardMultiplierMin <= resolved.rewardMultiplierMax &&
      resolved.minGoldReward > 0 &&
      resolved.minTroopsReward > 0;
    if (!finite || !coherent) {
      console.warn(
        "[vaultfront] invalid tuning values detected, rolling back to last valid tuning",
      );
      return base;
    }
    this.lastValidVaultTuning = resolved;
    return resolved;
  }
  vaultRuntimeExperiment(): VaultRuntimeExperiment | null {
    const exp = this._gameConfig.vaultRuntimeExperiment;
    if (!exp) return null;
    return {
      experimentId: exp.experimentId,
      rewardVariant: exp.rewardVariant,
      hudVariant: exp.hudVariant,
    };
  }
  vaultWeeklyMutator(): VaultWeeklyMutator {
    const mutator = this._gameConfig.vaultWeeklyMutator;
    if (
      mutator === "lane_fog" ||
      mutator === "accelerated_cooldowns" ||
      mutator === "double_passive"
    ) {
      return mutator;
    }
    return "none";
  }
  contractCardsEnabled(): boolean {
    return this._gameConfig.contractCardsEnabled ?? false;
  }
  nationGoldTroopEmphasis(): number {
    return within(this._gameConfig.nationGoldTroopEmphasis ?? 50, 0, 100);
  }

...
export class PlayerExecution implements Execution {
  ...
  private mg: Game;
  private active = true;
  private lastFocus = 50;
  private stableFocusTicks = 0;
  private focusFlipPenalty = 0;
  ...
  tick(ticks: number) {
    ...
    const focus = this.player.goldTroopFocus();
    this.mg.stats().resourceFocusSample(this.player, focus);
    ...
    const goldFromWorkers = BigInt(
      Math.max(0, Math.floor(Number(baseGoldFromWorkers) * goldMultiplier)),
    );
  ...
  private applyFocusDisciplineAdjustments(focus: number): number {
    if (focus === this.lastFocus) {
      this.stableFocusTicks = Math.min(2_400, this.stableFocusTicks + 1);
    } else {
      this.lastFocus = focus;
      this.stableFocusTicks = 0;
      this.focusFlipPenalty = Math.min(9, this.focusFlipPenalty + 1);
    }

    if (
      this.focusFlipPenalty > 0 &&
      this.stableFocusTicks > 0 &&
      this.stableFocusTicks % 90 === 0
    ) {
      this.focusFlipPenalty = Math.max(0, this.focusFlipPenalty - 1);
    }

    const stabilityBonus = Math.min(8, this.stableFocusTicks / 180);
    const penalty = this.focusFlipPenalty * 2;
    return Math.max(0, Math.min(100, focus + stabilityBonus - penalty));
  }

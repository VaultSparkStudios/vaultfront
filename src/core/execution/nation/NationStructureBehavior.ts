...
export class NationStructureBehavior {
  ...
  constructor(
    ...
    private nationGoldTroopEmphasis: number = 50,
  ) {}
  ...
  private shouldBuildStructure(
    ...
    const emphasis = this.nationGoldTroopEmphasis / 100;
    ...
    // 0 => troops focus, 100 => gold focus
    switch (type) {
      case UnitType.Port:
      case UnitType.Factory:
        ratio *= 0.7 + emphasis * 0.8;
        break;
      case UnitType.DefensePost:
      case UnitType.SAMLauncher:
        ratio *= 1.4 - emphasis * 0.7;
        break;
      case UnitType.MissileSilo:
        ratio *= 0.9 + emphasis * 0.4;
        break;
      default:
        break;
    }
  ...
  private getSaveUpTarget(): Gold {
    ...
    if (mirvEnabled) {
      ...
      const base = this.cost(UnitType.MIRV) + this.cost(UnitType.HydrogenBomb);
      return this.applySaveUpMultiplier(base);
    }
    if (hydroEnabled) {
      // Save up for 5 hydrogen bombs
      return this.applySaveUpMultiplier(this.cost(UnitType.HydrogenBomb) * 5n);
    }
    if (atomEnabled) {
      // Save up for 20 atom bombs
      return this.applySaveUpMultiplier(this.cost(UnitType.AtomBomb) * 20n);
    }
    // No nukes enabled, no need to save up
    return 0n;
  }

  private applySaveUpMultiplier(base: Gold): Gold {
    const emphasis = this.nationGoldTroopEmphasis / 100;
    // Troop focus saves less, gold focus saves more.
    const multiplier = 0.75 + emphasis * 0.7;

...
export type Intent =
  ...
  | KickPlayerIntent
  | TogglePauseIntent
  | SetResourceFocusIntent
  | VaultConvoyCommandIntent
  | DefenseFactoryCommandIntent
  | VaultRolePingIntent
...
export type SetResourceFocusIntent = z.infer<
  typeof SetResourceFocusIntentSchema
>;
export type VaultConvoyCommandIntent = z.infer<
  typeof VaultConvoyCommandIntentSchema
>;
export type DefenseFactoryCommandIntent = z.infer<
  typeof DefenseFactoryCommandIntentSchema
>;
export type VaultRolePingIntent = z.infer<typeof VaultRolePingIntentSchema>;
...
const VaultConvoyRewardTuningValuesSchema = z
  .object({
    minGoldReward: z.number().int().min(1),
    minTroopsReward: z.number().int().min(1),
    lowSignalEarlyWindowTicks: z.number().int().min(0),
    riskMultiplierBase: z.number().min(0),
    riskMultiplierScale: z.number().min(0),
    rewardMultiplierMin: z.number().min(0),
    rewardMultiplierMax: z.number().min(0),
    baselineGoldOwnerStrengthScale: z.number().min(0),
    baselineGoldAvgStrengthScale: z.number().min(0),
    baselineGoldRiskBase: z.number().min(0),
    baselineGoldRiskScale: z.number().min(0),
    distanceGoldMin: z.number().int().min(0),
    distanceGoldOwnerStrengthScale: z.number().min(0),
    distanceGoldFlat: z.number().min(0),
    distanceGoldRiskBase: z.number().min(0),
    distanceGoldRiskScale: z.number().min(0),
    troopsSqrtGoldScale: z.number().min(0),
    troopsDistanceBase: z.number().min(0),
    troopsDistanceRiskScale: z.number().min(0),
  })
  .partial();

const VaultConvoyRewardTuningVersionedSchema = z.object({
  version: z.literal("v1"),
  values: VaultConvoyRewardTuningValuesSchema,
});

const VaultRuntimeExperimentSchema = z
  .object({
...
const VaultWeeklyMutatorSchema = z
  .enum(["none", "lane_fog", "accelerated_cooldowns", "double_passive"])
  .optional();
...
export const GameConfigSchema = z.object({
  ...
  vaultSitesEnabled: z.boolean().optional(),
  intelOperationsEnabled: z.boolean().optional(),
  vaultConvoyRewardTuning: z
    .union([
      VaultConvoyRewardTuningValuesSchema,
      VaultConvoyRewardTuningVersionedSchema,
    ])
    .optional(),
  vaultRuntimeExperiment: VaultRuntimeExperimentSchema,
  vaultWeeklyMutator: VaultWeeklyMutatorSchema,
  contractCardsEnabled: z.boolean().optional(),
  nationGoldTroopEmphasis: z.number().min(0).max(100).optional(),
});
...
export const SetResourceFocusIntentSchema = z.object({
  type: z.literal("set_resource_focus"),
  focus: z.number().min(0).max(100),
});

export const VaultConvoyCommandIntentSchema = z.object({
  type: z.literal("vault_convoy_command"),
  command: z.union([
    z.literal("reroute_city"),
    z.literal("reroute_port"),
    z.literal("reroute_factory"),
    z.literal("reroute_silo"),
    z.literal("reroute_safest"),
    z.literal("escort"),
  ]),
});

export const DefenseFactoryCommandIntentSchema = z.object({
  type: z.literal("defense_factory_command"),
  command: z.union([z.literal("jam_breaker")]),
});

export const VaultRolePingIntentSchema = z.object({
  type: z.literal("vault_role_ping"),
  ping: z.union([
    z.literal("escort_convoy"),
    z.literal("intercept_lane"),
    z.literal("pulse_soon"),
  ]),
});
...
const IntentSchema = z.discriminatedUnion("type", [
  ...
  SetResourceFocusIntentSchema,
  VaultConvoyCommandIntentSchema,
  DefenseFactoryCommandIntentSchema,
  VaultRolePingIntentSchema,
...
export const GameRecordSchema = AnalyticsRecordSchema.extend({
  ...
  replay: z
    .object({
      snapshotIntervalTurns: z.number().int().positive(),
      snapshots: z.array(
        z.object({
          turnNumber: z.number().int().nonnegative(),
          hash: z.number().nullable(),
          intentsOffset: z.number().int().nonnegative(),
          intentsLength: z.number().int().nonnegative(),
        }),
      ),
      intentCounts: z.record(z.string(), z.number().int().nonnegative()),
    })
    .optional(),
});

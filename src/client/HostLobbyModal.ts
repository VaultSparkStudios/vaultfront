...
export class HostLobbyModal extends BaseModal {
  ...
  @state() private nationGoldTroopEmphasis: number = 50;
  @state() private vaultSitesEnabled: boolean = true;
  @state() private intelOperationsEnabled: boolean = true;
  @state() private contractCardsEnabled: boolean = false;
  ...
  render() {
    const inputCards = [
      ...
      ></toggle-input-card>`,
      html`<div
        class="col-span-2 rounded-xl p-4 flex flex-col justify-center border transition-all duration-200 bg-blue-500/20 border-blue-500/50 shadow-[0_0_15px_rgba(59,130,246,0.2)]"
      >
        <fluent-slider
          min="0"
          max="100"
          step="1"
          .value=${this.nationGoldTroopEmphasis}
          label="Nation Focus (0=Troops, 100=Gold)"
          @value-changed=${this.handleNationGoldTroopEmphasisChanged}
        ></fluent-slider>
      </div>`,
    ];
  ...
  protected onClose(): void {
    ...
    this.nationGoldTroopEmphasis = 50;
    this.vaultSitesEnabled = true;
    this.intelOperationsEnabled = true;
    this.contractCardsEnabled = false;
  ...
  private handleNationGoldTroopEmphasisChanged = (e: Event) => {
    const customEvent = e as CustomEvent<{ value: number }>;
    const value = Math.max(0, Math.min(100, customEvent.detail.value));
    this.nationGoldTroopEmphasis = value;
    this.putGameConfig();
  };
      ...
      new CustomEvent("update-game-config", {
        detail: {
          config: {
            ...
            nationGoldTroopEmphasis: this.nationGoldTroopEmphasis,
            vaultSitesEnabled: this.vaultSitesEnabled,
            intelOperationsEnabled: this.intelOperationsEnabled,
            contractCardsEnabled: this.contractCardsEnabled,

...
export class SinglePlayerModal extends BaseModal {
  ...
  @state() private nationGoldTroopEmphasis: number =
    DEFAULT_OPTIONS.nationGoldTroopEmphasis;
  ...
  protected onClose(): void {
    ...
    this.nationGoldTroopEmphasis = DEFAULT_OPTIONS.nationGoldTroopEmphasis;
  }
  ...
  private handleNationGoldTroopEmphasisChanged = (e: Event) => {
    const customEvent = e as CustomEvent<{ value: number }>;
    this.nationGoldTroopEmphasis = Math.max(
      0,
      Math.min(100, customEvent.detail.value),
    );
  };

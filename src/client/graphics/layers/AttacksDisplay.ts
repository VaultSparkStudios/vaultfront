...
export class AttacksDisplay extends LitElement implements Layer {
          ...
          ${this.renderButton({
            content: html`<img
              ...
              <span class="inline-block min-w-[3rem] text-right"
                >${this.formatIncomingAttackTroops(attack)}</span
              >
  ...
  private formatBoatTroops(boat: UnitView): string {
    const owner = boat.owner();
    if (owner.isTroopIntelHiddenToViewer()) {
      return "??";
    }
    return renderTroops(boat.troops());
  }
        ...
        >
          ${this.renderButton({
            content: html`${this.renderBoatIcon(boat)}
              <span class="inline-block min-w-[3rem] text-right"
                >${this.formatBoatTroops(boat)}</span
              >
  ...
  render() {
    ...
    return html`
      <div
        class="fixed left-1/2 bottom-[4.75rem] z-[1090] flex w-[min(92vw,440px)] -translate-x-1/2 flex-col gap-1 overflow-y-auto rounded-lg text-white text-sm lg:text-base min-[1200px]:bottom-[5.25rem] max-h-[24vh]"
      >
